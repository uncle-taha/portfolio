...
const translations = {
  ...
  ko: {
    ...
    p2: `숫자 0은 문자 O와 D와 비슷하게 보이고, 숫자 1은 문자 I과 닮아 시각적으로 혼동될 수 있습니다. 이는 배달 라이더에게 혼란을 주고 때로는 잘못된 배달로 이어질 수 있습니다.`,
    itelicP: `"혹자는 배달 라이더가 더 주의해야 한다고 말합니다."`,
    p3: `원칙적으로는 맞지만, 피크 시간이나 나쁜 기상 조건(폭우, 폭설)에서는 집중하기가 훨씬 어렵습니다. 라이더로 일하면서 이런 문제를 자주 겪었고, 그럴 때마다 고객과의 실랑이로 돈과 시간을 낭비했습니다. 이것은 결과적으로 라이더만의 문제가 아니라 배민에 대한 고객 불만족으로 이어집니다.`,
    p4: `QR 코드를 이용한 주문 픽업 시스템은 고객, 라이더, 식당 주인 모두에게 이롭습니다. 배민이 이미 매장에 QR 시스템을 사용하고 있지만, 모든 주문에 적용해야 합니다. 또 다른 걱정은`,
    p4italic: `"QR 코드를 백엔드에 저장해야 해서 공간을 차지하고 비용이 증가한다는 점입니다."`,
    p4cont: `실제로는 데이터베이스에 QR 코드를 저장할 필요가 없습니다. 주문 번호 12자리가 이미 데이터베이스에 저장되어 있기 때문에, 식당 영수증 시스템에 클라이언트 사이드에서 QR 코드를 생성하는 코드 몇 줄이면 충분합니다. 스캔하면 QR은 주문 번호를 보여주고, 라이더의 주문 번호와 일치하면 픽업이 성공적으로 확인됩니다. 저는 데이터베이스 없이 작동하는 QR 코드 생성기를 만들었으며, `,
    p4linkText: "여기서 확인해볼수 있습니다",
    p5Title: `"마이 페이지"에서 수락률을 왜 제거해야 할까요?`,
    p5a: `운전자에게 새 주문을 받으면 수락 또는 거절할 수 있는 시간이 약 1분 정도 주어집니다. 라이더가 주문을 받고 싶지 않으면 두 가지 선택지가 있습니다:
`,
    p5List1: `주문을 거절하면 수락률에 영향을 줍니다 (배차 수락률)`,
    p5List2: `주문이 자동으로 사라질 때까지 1분 기다립니다`,
    p5b: `운전자가 너무 많은 주문을 거절하면 수락률에 부정적인 영향이 생깁니다. 수락률이 낮으면 AI 추천 주문을 받을 가능성이 줄어듭니다. 때문에 일부 운전자는 수락률을 낮추지 않으려고 자동으로 주문이 사라질 때까지 기다립니다. 이로 인해 고객에게 늦게 도착하는 문제가 발생합니다.`,
    p5c: `비가 오거나 피크 시간에는 문제가 더 심해집니다. 주문이 10명의 운전자에게 보여져도 아무도 수락하지 않으면 주문이 이미 10분 늦어진 것입니다.`,
